...
function hasLabel(labels: (string | { name?: string; })[], label: string): boolean {
...
function issueBody(issue: { body?: string; }, comments: { body?: string; }[]): string {
...
async function run(): Promise<void> {
  ...
  const { data: comments } = await octokit.rest.issues.listComments(octokitArgs);
